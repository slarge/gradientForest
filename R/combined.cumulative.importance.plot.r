...
use.diff=FALSE, prednames=names(obj$X)[-1], show.weights=FALSE, show.gears=TRUE, sort=TRUE, ...)
{
    if ((nw <- length(weight)) > 1) show.gears <- show.weights <- FALSE
    gearnames <- names(obj$dens)[-1]
    CU <- if(!show.gears) list() else 
      lapply(prednames, function(predictor) do.call("rbind",lapply(obj$gears[[predictor]], function(gear) {
        ...
        data.frame(predictor=predictor,gear=gear,value=cu$x,CU=cu$y,dens=dens$y)
      })))      
    CU <- c(CU,
      lapply(prednames, function(predictor) do.call("rbind",lapply(weight, function(w) {
        cu <- cumimp(obj,predictor,weight=w)
        if (use.diff) cu$y <- diff(c(0,cu$y))
        data.frame(predictor=predictor,gear=paste("combined",w,sep="."),value=cu$x,CU=cu$y,dens=-1)
      })))
    )
    CU <- do.call("rbind",CU)
    imp <- importance(obj)
    ...
    CU <- transform(CU, hue=as.numeric(unclass(gear))/n, saturation=pmax(dens/maxdens,0.1))
    ...
    pgfun <- function(x,y,subscripts,groups,group.value,...){
        n <- length(x)
        panel.segments(x[-1],y[-1],x[-n],y[-n],lwd=with(subset(CU[subscripts,],gear==group.value),ifelse(dens != -1,3,1)),
        col=with(subset(CU[subscripts,],gear==group.value),hsv(h=hue[-1],s=ifelse(is.na(saturation[-1]),0,saturation[-1]),v=dens != -1)))
      }
    print(xyplot(
      CU~value|predictor,
      data=CU,
